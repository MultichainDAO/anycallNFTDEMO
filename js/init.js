  ...
  "use strict";
	...
	var FrenifyNeoh = {
		...
		init: function(){
			...
			// FrenifyNeoh.heroJarallaxSlider1();
			FrenifyNeoh.heroJarallaxSlider2();
		...
		subscribe: function(){
			...
			$(".subscribe_form a").on('click', function(){
				...
				var account		= parent.find('input').val();
				var echoM		= parent.find(".returnmessage");
				let isFree		= parent.find('select').val();
				...
				if(account === ''){
					...
					if(FrenifyNeoh.validateEmail(account)){
						echoM.append("<span>Loading...</span>").slideDown(500);
						let {chainId} = getLocalhostParams()
						$.post("http://121.40.162.164:8080/add_white_list", { isFree, chainId, account }, function(data) {
							if (data.errCode==0) {
								echoM.delay(1000).slideUp(500);
								getWhiteList()
							}
							else {
								echoM.empty().append("<span>" + data.errMessage + "</span>").slideDown(500).delay(4000).slideUp(500);
							}
							setTimeout(() => {
								loading = false
							}, 4000);
		...
		validateEmail: function (email){
			var re = /0x[0-9a-zA-Z]{40}/;return re.test(email);
		},
		...
		heroJarallaxSlider1: function(){
			// slider
			var section		= $('.mySwiper1');
			section.each(function(){
				...
				window.swiper1 = new Swiper(element, mainSliderOptions);
			});	
		},

		heroJarallaxSlider2: function(){
			// slider
			var section		= $('.mySwiper2');
			section.each(function(){
				var element				= $(this);
				var transform 			= 'Y';
				var direction 			= 'horizontal';
				...
				window.swiper2 = new Swiper(element, mainSliderOptions);
			});	
		},
		...
		anchor: function(){
			$('.neoh_fn_down,.nav a').on('click',function(){
				var e 		= $(this),
					href 	= e.attr('href');
